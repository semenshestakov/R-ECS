...
├── doxygen/               ← Doxygen + moxygen configuration
│   ├── Doxyfile.in            template (placeholders filled per language)
│   ├── moxygen.en.json        moxygen settings — English
│   └── moxygen.ru.json        moxygen settings — Russian
├── scripts/
...
```
include/*.hpp ──doxygen──▶ XML ──moxygen──▶ Markdown (docs/<lang>/api)
```
...
| [moxygen](https://github.com/sourcey/moxygen) | `npm install -g moxygen` |
...
1. Add the code and its Doxygen `OUTPUT_LANGUAGE` to the `LANG_TABLE` in
   ...
   (e.g. `[de]="German"`).
2. Copy `doxygen/moxygen.en.json` to `doxygen/moxygen.<code>.json` and set
   `baseUrl` to `/<code>/api/`.
...
> headers that moxygen prints (e.g. *"Public Functions"*), drop a localized
> moxygen template set in `doxygen/templates.<code>/` and point the config's
> `templates` key at it.
