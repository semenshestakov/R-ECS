...
выдаёт обёртку *и* фильтрует по своему биту. Создание сущности через
`Create<Door>(...)` выставляет этот бит автоматически; при желании его можно
навесить как обычный тег — через `AddTag<Door>()`.
...
Door door = world.Create<Door>(std::move(prefab)); // Create<Door> сам ставит бит Door

for (auto [d] : world.view<Door>())              // выдаёт Door, фильтрует по биту Door
    d.GetPosition().x += 1.f;
```
...
> Именованная обёртка фильтрует по своему тегу, поэтому сущность попадёт в
> `view<Door>()` только когда несёт бит `Door`. `Create<Door>(...)` выставляет этот
> бит при создании — тег обёртки добавляется в архетип, не меняя исходный префаб
> (префаб, переиспользованный для сущности другого типа, остаётся без тега). Сущности,
> созданной как `Create<Entity>(...)`, по-прежнему нужен явный `AddTag<Door>()`, чтобы
> попасть в представление.
