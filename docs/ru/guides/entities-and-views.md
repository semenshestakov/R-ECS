...
Поставьте `Entity` первым в списке аргументов, чтобы дополнительно получать хэндл
владеющей сущности первым элементом каждого шага. `Entity` не является
компонентом — он исключается из фильтра по архетипу, поэтому представление
по-прежнему отбирает сущности только по остальным компонентам:
