...
Поскольку порядок обработки событий выводится из расписания (более ранние стадии
получают более высокий приоритет), `Registry::Update` обновляет порядок событий
всякий раз, когда расписание было пересчитано. Существующие обработчики **не**
переподписываются: их приоритеты меняются на месте через `SetEventsPriority`
(который вызывает `Event::setPriority`), поэтому порядок диспетчеризации следует
за новыми стадиями без лишней работы. Системы, добавленные после первой подписки,
подписываются один раз при следующем обновлении.
...
Теги доступа: `ecs::Read<C>` (чтение) и `ecs::Write<C>` (запись). Из объявленного
доступа расписание выводит `Data`-рёбра так, что:
...
Выключенная система перестаёт получать вызовы `Update()` **и** её обработчики
событий отсоединяются, поэтому она больше не реагирует и на события; повторное
включение переподписывает их. Сверка происходит в начале следующего `Update()`.
...
пропускается и отписывается от событий вместе с ней. Системы, связанные только
доступом к компонентам или через `ECS_WEAK_DEPENDENT_SYSTEMS`, продолжают работать.
...
Эти вызовы доступны только из главного потока. Чтобы переключить систему изнутри
другой системы, работающей на рабочем потоке, отложите это через очередь команд с
помощью [`DisableSystemCmd` / `EnableSystemCmd`](./commands.md#disablesystemcmd--enablesystemcmd),
которые применяют изменение на потоке Registry при следующем сбросе.
...
Системы одной стадии диспатчатся через планировщик задач и могут выполняться
параллельно; см. [Джобы и многопоточность](./jobs.md#параллельный-запуск-систем).

## См. также

- [Джобы и многопоточность](./jobs.md) — как стадии запускаются параллельно.
