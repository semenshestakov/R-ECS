...
namespace ecs
{
    ...
    struct IJobScheduler
    {
         ...
         *
         * The cross-archetype chunk iterator is forward-only, so this method first does one cheap
         * pass to gather the matched chunks into a contiguous array — at which point the chunk
         * **count is known** and supports random access. That is exactly what lets it drive this
         * port's own index-based ParallelFor (which recursively bisects [0, N) with a grain and
         * work-steals) instead of a forward-only `parallel_for_each`: better load balancing, no
         * serial feeder. The gather is O(number of chunks) — i.e. entities / chunk capacity — so
         * it is negligible against the per-entity work.
         *
         ...
         *
         * Defined on the port itself, it is backend-agnostic: it runs truly in parallel on
