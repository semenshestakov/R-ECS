...
namespace
{
    ...
    TEST_F(TbbJobSchedulerTest, ParallelForVisitsEveryIndexExactlyOnce)
    {
        ...
        EXPECT_TRUE(std::all_of(hits.begin(), hits.end(), [](const int h) { return h == 1; }));
    }
    ...
    TEST_F(TbbJobSchedulerTest, ParallelForExplicitGrainStillCoversRange)
    {
        ...
        scheduler.ParallelFor(0, n, 100, [&](const std::size_t first, const std::size_t last) {
            count += (last - first);
        });
    ...
    TEST_F(TbbJobSchedulerTest, WaitOnEmptyHandleIsNoOp)
    {
        EXPECT_NO_THROW(scheduler.Wait(ecs::JobHandle{}));
    }
    ...
    TEST_F(TbbJobSchedulerTest, WorkerCountIsAtLeastOne)
    {
        EXPECT_GE(scheduler.WorkerCount(), 1u);
    }

    TEST_F(TbbJobSchedulerTest, HostsDedicatedThreads)
    {
        EXPECT_TRUE(scheduler.CanHostDedicatedThreads());
    }

    TEST(TbbJobSchedulerCtorTest, ExplicitThreadCountBoundsWorkerCount)
    {
        ecs::TbbJobScheduler scheduler{2};
        EXPECT_GE(scheduler.WorkerCount(), 1u);
    }
    ...
    // Spin until `value` reaches `target` or the timeout elapses, so the test does
    // not depend on a fixed sleep being long enough on a loaded machine.
    template <typename T>
    bool WaitUntilAtLeast(const std::atomic<T>& value, const T target,
    ...
    TEST_F(TbbJobSchedulerTest, ServiceTicksUntilStopped)
    {
        ...
        EXPECT_EQ(ticks.load(), after); // no ticks after the join in StopService
    }
    ...
    TEST_F(TbbJobSchedulerTest, ServiceDescIsAccepted)
    {
        std::atomic<int> ticks{0};
        const ecs::ServiceHandle svc = scheduler.SpawnService(
            [&] { ++ticks; std::this_thread::sleep_for(1ms); },
    ...
    // Lifetime invariant: destroying the scheduler stops and joins services even
    // when the owner never called StopService.
    TEST(TbbJobSchedulerLifetimeTest, DestructionStopsRunningServices)
    {
        auto ticks = std::make_shared<std::atomic<int>>(0);
        {
            ecs::TbbJobScheduler scheduler;
            // Intentionally drop the handle: the scheduler still owns the service,
            // so only its destructor can stop it.
            static_cast<void>(scheduler.SpawnService([ticks] {
                ++*ticks;
                std::this_thread::sleep_for(1ms);
            }));
            EXPECT_TRUE(WaitUntilAtLeast(*ticks, 3));
            // leave scope without StopService -> destructor must stop & join
        }
        // If the destructor had not joined, the worker thread would still be
        // touching `ticks`; capture a baseline and confirm it is now frozen.
